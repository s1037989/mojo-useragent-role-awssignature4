...
my $ec2 = 'https://ec2.us-east-1.amazonaws.com?Action=DescribeVolumes';
...
$ec2 = 'https://ec2.us-east-1.amazonaws.com?Action=DescribeVolumes';
$tx = $ua->awssig4($awssig4)->build_tx(GET => $ec2);
is   $ua->awssig4->service, 'ec2', 'right non-default service';
is   $ua->awssig4->expires, 600, 'right non-default expires';
...
SKIP: {
  ...
  $tx = $ua->start($tx);
  is $tx->req->headers->header('X-Amz-Expires'), undef, 'right default expires';
  ok $tx->res->dom->at('requestId')->text, 'has a requestId';
  my $rid1 = $tx->res->dom->at('requestId')->text;
  $tx = $ua->get($ec2);
  ok $tx->res->dom->at('requestId')->text, 'has a requestId';
  my $rid2 = $tx->res->dom->at('requestId')->text;
